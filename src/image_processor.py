...
import time
import numpy as np
from .heart_rate_monitor import HeartRateEvaluation
...
def face_processing(
    ...
    hr_eval_obj = HeartRateEvaluation(control_obj)
    ...
    live = settings["video_source"]["live"]
    evaluate_dataset = settings["video_source"]["evaluate_dataset"]

    if evaluate_dataset:
        path_measurements = settings["video_source"]["path_measurements"]
        csv_rows = hr_eval_obj.read_csv_to_list(path_measurements)
    ...
    with mp_face_mesh.FaceMesh(
        ...
        for i, frame in enumerate(video_stream.update()):
            # Calculate FPS
            current_time = time.time()
            if evaluate_dataset and not live:
                csv_row = csv_rows[i] if i < len(csv_rows) else None
                if not all(value == "" for value in csv_row.values()):
                    hr_eval_obj.extract_data_dataset(csv_row)
            ...
            if results.multi_face_landmarks:
                ...
                for face in results.multi_face_landmarks:
                    ...
                    # durr = time.time() - current_time
                    # if len(time_array) < iterations:
                    #    time_array.append(durr)
                    # if len(time_array) == iterations:
                    #    print("average time: ", np.mean(time_array), "\n")
                    #    print("standard deviation: ", np.std(time_array), "\n")
